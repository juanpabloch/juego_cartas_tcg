...
class PlayerZones:
    ...
    def __init__(self, cards, treasures) -> None:
    ...
    def retornar_tesoros_agotados(self):
        self.move_all_cards(self.tesoros_agotados, self.reserva_tesoros)
    
    def retornar_unidades_a_formacion(self):
        self.move_all_cards(self.combate, self.formacion)
...
class PlayerActions:
    ...
    def draw_card(self, count=1):
        drawn = self.zones.mazo.remove_amount(count)
        leftovers = self.zones.hand.add_cards(drawn)
        
        if leftovers:
            self.zones.mazo.cards = leftovers + self.zones.mazo.cards
        
    def play_card(self, card_id):
        card = self.zones.hand.get_card_by_id(card_id)

        if not card:
            return False
        
        if card.can_be_played(self.resources.available_gold):
            # Coordina: mover carta + gastar oro
            self.zones.move_card(self.zones.hand, self.zones.formacion, card_id)
    ...
    def first_turn_return_card_to_bottom(self, card_id):
        self.zones.move_card_to_bottom(self, self.zones.hand, self.zones.mazo, card_id)
    
    
    def mulligan(self):
        self.zones.move_all_cards(self.zones.hand, self.zones.mazo)
...
class Player:
    def __init__(self, name, cards, treasures):
        self.name = name
        self.resources = PlayerResources()
        self.zones = PlayerZones(cards, treasures)
